...
describe('Invitation accept API', () => {
  ...
  test('accepts invitation for signed-in user with matching email', async () => {
    const singleResponses: Array<SupabaseRow<unknown>> = [
      { data: { id: 'inv-1', campaign_id: 'camp-1', email: 'a@ex.com', role: 'co-gm', accepted: false }, error: null },
  ...
  fakeSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user-1', email: 'a@ex.com' } }, error: null } as { data: { user: { id: string; email?: string } } | null; error: unknown })
    ...
    const req = { json: async () => ({ token: 't1' }) } as unknown as Parameters<typeof acceptPOST>[0]
  ...
  fakeSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user-2', email: 'other@ex.com' } }, error: null } as { data: { user: { id: string; email?: string } } | null; error: unknown })
  ...
  test('returns 410 when invitation is expired', async () => {
    ...
    const singleResponses: Array<SupabaseRow<unknown>> = [
      { data: { id: 'inv-1', campaign_id: 'camp-1', email: 'a@ex.com', role: 'co-gm', accepted: false, expires_at: past }, error: null },
    ]
  ...
  fakeSupabase.auth.getUser.mockResolvedValue({ data: { user: { id: 'user-1', email: 'a@ex.com' } }, error: null } as { data: { user: { id: string; email?: string } } | null; error: unknown })
    ...
    expect((res as unknown as { status: number }).status).toBe(410)
    expect((res as unknown as { body: unknown }).body).toEqual({ error: 'Invitation expired' })
  })
})
